...
#define LOG_NDEBUG 0
#define LOG_TAG "SimpleHardOMXComponent"
#include <utils/Log.h>
...
SimpleHardOMXComponent::SimpleHardOMXComponent(
        const char *name,
        const OMX_CALLBACKTYPE *callbacks,
        OMX_PTR appData,
        OMX_COMPONENTTYPE **component)
    : HardOMXComponent(name, callbacks, appData, component),
      mLooper(new ALooper),
      mHandler(new AHandlerReflector<SimpleHardOMXComponent>(this)),
...
void SimpleHardOMXComponent::prepareForDestruction() {
...
OMX_ERRORTYPE SimpleHardOMXComponent::sendCommand(
...
bool SimpleHardOMXComponent::isSetParameterAllowed(
...
OMX_ERRORTYPE SimpleHardOMXComponent::getParameter(
        OMX_INDEXTYPE index, OMX_PTR params) {
    Mutex::Autolock autoLock(mLock);
    return internalGetParameter(index, params);
}

OMX_ERRORTYPE SimpleHardOMXComponent::setParameter(
...
OMX_ERRORTYPE SimpleHardOMXComponent::internalGetParameter(
        OMX_INDEXTYPE index, OMX_PTR params) {
    switch (index) {
        case OMX_IndexParamPortDefinition:
        {
...
OMX_ERRORTYPE SimpleHardOMXComponent::internalSetParameter(
        OMX_INDEXTYPE index, const OMX_PTR params) {
    switch (index) {
        case OMX_IndexParamPortDefinition:
        {
...
OMX_ERRORTYPE SimpleHardOMXComponent::useBuffer(
...
OMX_ERRORTYPE SimpleHardOMXComponent::allocateBuffer(
        OMX_BUFFERHEADERTYPE **header,
        OMX_U32 portIndex,
        OMX_PTR appPrivate,
        OMX_U32 size) {
...
OMX_ERRORTYPE SimpleHardOMXComponent::freeBuffer(
        OMX_U32 portIndex,
        OMX_BUFFERHEADERTYPE *header) {
    Mutex::Autolock autoLock(mLock);
...
OMX_ERRORTYPE SimpleHardOMXComponent::emptyThisBuffer(
...
OMX_ERRORTYPE SimpleHardOMXComponent::fillThisBuffer(
        OMX_BUFFERHEADERTYPE *buffer) {
...
OMX_ERRORTYPE SimpleHardOMXComponent::getState(OMX_STATETYPE *state) {
    Mutex::Autolock autoLock(mLock);
...
void SimpleHardOMXComponent::onMessageReceived(const sp<AMessage> &msg) {
    Mutex::Autolock autoLock(mLock);
...
void SimpleHardOMXComponent::onSendCommand(
...
void SimpleHardOMXComponent::onChangeState(OMX_STATETYPE state) {
...
void SimpleHardOMXComponent::onPortEnable(OMX_U32 portIndex, bool enable) {
    CHECK_LT(portIndex, mPorts.size());
...
void SimpleHardOMXComponent::onPortFlush(
...
void SimpleHardOMXComponent::checkTransitions() {
...
void SimpleHardOMXComponent::addPort(const OMX_PARAM_PORTDEFINITIONTYPE &def) {
...
void SimpleHardOMXComponent::onQueueFilled(OMX_U32 portIndex) {
}

void SimpleHardOMXComponent::onPortFlushCompleted(OMX_U32 portIndex) {
}

void SimpleHardOMXComponent::onPortEnableCompleted(
        OMX_U32 portIndex, bool enabled) {
}

List<SimpleHardOMXComponent::BufferInfo *> &
SimpleHardOMXComponent::getPortQueue(OMX_U32 portIndex) {
    CHECK_LT(portIndex, mPorts.size());
    return mPorts.editItemAt(portIndex).mQueue;
}

SimpleHardOMXComponent::PortInfo *SimpleHardOMXComponent::editPortInfo(
        OMX_U32 portIndex) {
    CHECK_LT(portIndex, mPorts.size());
