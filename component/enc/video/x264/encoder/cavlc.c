...
static int block_residual_write_cavlc( x264_t *h, bs_t *s, int i_ctxBlockCat, int16_t *l, int nC )
{
  ...
  //static const uint8_t ctz_index[8] = {3,0,1,0,2,0,1,0};
    ...
    //END(ones);
    //if(ones_calls == 3600 )
    // printf("i_trailing0 = %d, i_sign0 = %u\n", i_trailing, i_sign);
    //FORMAT_PRINT(ones);
#endif

    //runlevel.level[2] = 2;  
    //runlevel.level[1] = 2;
    /* The previous level[16] won't be erased: need above 2 assignments OR 
       the following i_total test to avoid interfereing of i_trailing calculation.
     */
    //START(ones_fi); // around 190 pmon until trailing  
    i_total = h->quantf.coeff_level_run[i_ctxBlockCat]( l, &runlevel );
    i_total_zero = runlevel.last + 1 - i_total;

    if( runlevel.level[0] != 1 && runlevel.level[0] != -1 )    
    {
        i_trailing = 0;
        i_sign = 0;
    }
    else if( i_total < 2 || (runlevel.level[1] != 1 && runlevel.level[1] != -1 ) )
    {
        i_trailing = 1;
        i_sign = (runlevel.level[0] >> 31) & 1;
    }  
    else if( i_total < 3 || (runlevel.level[2] != 1 && runlevel.level[2] != -1 ) )
    {
        i_trailing = 2;
        i_sign = ((runlevel.level[0] >> 31) & 2) 
               | ((runlevel.level[1] >> 31) & 1);
    }
    else
    {
        i_trailing = 3;
        i_sign = ((runlevel.level[0] >> 31) &4) 
               | ((runlevel.level[1] >> 31) &2) 
               | ((runlevel.level[2] >> 31) &1);
    }
    //printf("i_trailing--1 = %d, i_sign--1 = %u\n", i_trailing, i_sign);
    //END(ones_fi);
    //FORMAT_PRINT(ones_fi); 
...
{\
    ...
    uint8_t *nnz = &entropy_var_ptr->cache.non_zero_count[x264_scan8[idx]];\
    if( !*nnz )\
        bs_write_vlc( s, x264_coeff0_token[nC] );\
    else{ \
        *nnz = block_residual_write_cavlc(h,s,cat,l,nC);\
	/*h->mb.cache.non_zero_count[x264_scan8[idx]] = *nnz;*/ /*need for save?*/ \
    }\
}
#else //ENTROPY_PIPE
#define block_residual_write_cavlc(h,s,cat,idx,l)\
...
{
#ifdef ENTROPY_PIPEed
  //const int i_mb_type = entropy_var_ptr->i_type;
    const int i_mb_type = h->mb.i_type;

    //const int i_cbp_luma = entropy_var_ptr->i_cbp_luma;
    //const int i_cbp_chroma = entropy_var_ptr->i_cbp_chroma;
    const int i_cbp_luma = h->mb.i_cbp_luma;
    const int i_cbp_chroma = h->mb.i_cbp_chroma;


    //int i_dqp = entropy_var_ptr->i_qp - h->mb.i_last_qp;
    int i_dqp = h->mb.i_qp - h->mb.i_last_qp;
    //uint8_t nnz_scan8_24 = entropy_var_ptr->cache.non_zero_count[x264_scan8[24]];
    uint8_t nnz_scan8_24 = h->mb.cache.non_zero_count[x264_scan8[24]];
    //int *i_qp_ptr = &entropy_var_ptr->i_qp;
    int *i_qp_ptr = &h->mb.i_qp;
#else
    const int i_mb_type = h->mb.i_type;
    ...
    if( i_mb_type == I_16x16 && !(i_cbp_luma | i_cbp_chroma) && !nnz_scan8_24 )
    {
#if !RDO_SKIP_BS
        *i_qp_ptr = h->mb.i_last_qp;
#endif
        i_dqp = 0;
    }
...
#ifdef ENTROPY_PIPEed
    bs_write_se( s, entropy_var_ptr->cache.mv[i_list][x264_scan8[idx]][0] - mvp[0] );
    bs_write_se( s, entropy_var_ptr->cache.mv[i_list][x264_scan8[idx]][1] - mvp[1] );
#else
    bs_write_se( s, h->mb.cache.mv[i_list][x264_scan8[idx]][0] - mvp[0] );
    bs_write_se( s, h->mb.cache.mv[i_list][x264_scan8[idx]][1] - mvp[1] );
#endif
    //printf("mvx = %d, mvy = %d\n\n", 
    //   h->mb.cache.mv[i_list][x264_scan8[idx]][0], h->mb.cache.mv[i_list][x264_scan8[idx]][1]);
}
    ...
    for( i8 = i8start; i8 <= i8end; i8++ )
        if( h->mb.i_cbp_luma & (1 << i8) )
	      ...
	      block_residual_write_cavlc( h, s, DCT_LUMA_4x4, i4+i8*4, h->dct.luma4x4[i4+i8*4] );
              //block_residual_write_cavlc( h, s, DCT_LUMA_4x4, i4+i8*4, entropy_var_ptr->dct.luma4x4[i4+i8*4] );
}
...
void x264_macroblock_write_cavlc( x264_t *h, bs_t *s )
{
  //const int i_mb_type = entropy_var_ptr->i_type;
...
#if 0
    const int i_intra16x16_pred_mode = entropy_var_ptr->i_intra16x16_pred_mode;
    const int i_chroma_pred_mode = entropy_var_ptr->i_chroma_pred_mode;
#else
    const int i_intra16x16_pred_mode = h->mb.i_intra16x16_pred_mode;
    const int i_chroma_pred_mode = h->mb.i_chroma_pred_mode;
#endif
    ...
    /* Write:
      - type
      - mv */
    if( i_mb_type == I_16x16 )//I_SLICE
    {
        bs_write_ue( s, i_mb_i_offset + 1 + x264_mb_pred_mode16x16_fix[i_intra16x16_pred_mode] + 
		        i_cbp_chroma * 4 + ( i_cbp_luma == 0 ? 0 : 12 ) );
        bs_write_ue( s, x264_mb_pred_mode8x8c_fix[ i_chroma_pred_mode ] );
    }
    else //if( i_mb_type == P_L0 )//P_SLICE
    {
        //if( i_partition == D_16x16 )
        {
            bs_write1( s, 1 );

	    //bs_write_se( s, entropy_var_ptr->mvd[0] );
	    //bs_write_se( s, entropy_var_ptr->mvd[1] );	    
	    bs_write_se( s, h->mb.mvd_hw[0] );
	    bs_write_se( s, h->mb.mvd_hw[1] );
        }

        /* Coded block patern */
        //if( i_mb_type != I_16x16 )
            bs_write_ue( s, inter_cbp_to_golomb[( i_cbp_chroma << 4 ) | i_cbp_luma] );
	...
	//block_residual_write_cavlc( h, s, DCT_LUMA_DC, 24 , entropy_var_ptr->dct.luma16x16_dc );

        /* AC Luma */
        if( i_cbp_luma )
            for( i = 0; i < 16; i++ )
	      block_residual_write_cavlc( h, s, DCT_LUMA_AC, i, h->dct.luma4x4[i]+1 );
	      //block_residual_write_cavlc( h, s, DCT_LUMA_AC, i, entropy_var_ptr->dct.luma4x4[i]+1 );
    }
    else if( i_cbp_luma | i_cbp_chroma )
    {
        cavlc_qp_delta( h, s );
    ...
    if( i_cbp_chroma )
    {
        ...
        //block_residual_write_cavlc( h, s, DCT_CHROMA_DC, 25, entropy_var_ptr->dct.chroma_dc[0] );
        block_residual_write_cavlc( h, s, DCT_CHROMA_DC, 26, h->dct.chroma_dc[1] );
        //block_residual_write_cavlc( h, s, DCT_CHROMA_DC, 26, entropy_var_ptr->dct.chroma_dc[1] );
        if( i_cbp_chroma&0x02 ) /* Chroma AC residual present */
            for( i = 16; i < 24; i++ )
	      block_residual_write_cavlc( h, s, DCT_CHROMA_AC, i, h->dct.luma4x4[i]+1 );
	      //block_residual_write_cavlc( h, s, DCT_CHROMA_AC, i, entropy_var_ptr->dct.luma4x4[i]+1 );
    }
...
void x264_macroblock_write_cavlc( x264_t *h, bs_t *s )
{
    ...
    int i_intra16x16_pred_mode = h->mb.i_intra16x16_pred_mode;
    int i_chroma_pred_mode = h->mb.i_chroma_pred_mode;

    int i_skip_intra = h->mb.i_skip_intra;
    int b_skip_mc = h->mb.b_skip_mc;
