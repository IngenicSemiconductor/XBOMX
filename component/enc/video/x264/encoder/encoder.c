...
#include "soc/jz47_vae_map.h"
#include <jzmedia.h>
#include <jzasm.h>
#include <jzm_vpu.h>

// common include files
#include "soc/t_intpid.h"
#include "soc/jzm_x264_enc.h"
#include "soc/jzm_x264_enc.c"
#include "soc/tile.c"
#include "soc/pixel_check.c"
...
extern volatile unsigned char * efe_base;
...
#ifdef JZC_PMON_P0
//#include "soc/jz4760e_pmon.h"   //maybe defined in soc/config_jz_soc.h
PMON_CREAT(copy);
PMON_CREAT(hw);
#endif //JZC_PMON_P0

#ifdef CRC_CHECK
#include "soc/crc_check.c"
extern int bs_crc;
extern int dec_y_crc;
extern int dec_c_crc;
extern int bs_total_len;
#endif

extern void x264_frame_deblock_mb( x264_t *h, int mb_x, int mb_y );
extern void *jz4740_alloc_frame (int *VpuMem_ptr, int align, int size);
                                    ...
                                    int i_idr_pic_id, int i_frame, int i_qp )
{
    ...
    sh->b_field_pic = 0;    /* no field suppoprt for now */
...
static int x264_validate_parameters( x264_t *h )
{
    ...
    //add X264_CSP_YUYV for android camera; by gjwang
    if( h->param.i_csp != X264_CSP_I420 && h->param.i_csp != X264_CSP_YV12 && h->param.i_csp != X264_CSP_YUYV )
    {
    ...
    /* Detect default lume settings and terminate with an error. */
    {
        int score = 0;
        ...
        if( score >= 5 )
        {
            x264_log( h, X264_LOG_ERROR, "broken lume default settings detected\n" );
 ...
 * HW 4780 init:
 ****************************************************************************/
static int x264_init_jz4780(x264_t * h)
{
    volatile uint8_t *ptr;
    int width = h->sps->i_mb_width;
    int height = h->sps->i_mb_height;

    h->hwinfo = malloc(sizeof(HwInfo_t));
    HwInfo_t * s = (HwInfo_t *)h->hwinfo;

    s->H264E_SliceInfo.mb_width = width;
    s->H264E_SliceInfo.mb_height = height;
    ...
    //fprintf(stderr, "H264E_SliceInfo.fb[2]: %08x, %08x\n", s->H264E_SliceInfo.fb[2][0], s->H264E_SliceInfo.fb[2][1]);

    // BS
    ptr = (uint8_t *)jz4740_alloc_frame(h->param.VpuMem_ptr, 256, (0x1<<20) );
    s->bs_ptr = (uint8_t *)ptr;
    s->H264E_SliceInfo.bs = s->bs_ptr;
    //fprintf(stderr, "H264E_SliceInfo.bs: %08x\n", s->H264E_SliceInfo.bs);

    // vdma space
    s->vdma_config = (unsigned int *)jz4740_alloc_frame(h->param.VpuMem_ptr, 128, 0x5000);
    //fprintf(stderr, "vdma_config: %08x\n", (unsigned int)s->vdma_config);
    if( s->vdma_config == NULL ){
        printf("alloc vdma_config error!\n");
        return -1;
    }
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    //h->frames.b_have_lowres = 0;//gjwang
...
static void dump_frame_data( x264_t * h, uint8_t * Y, uint8_t * C )
{
    int i, j;
    int mb_w, mb_h;
...
static int x264_slice_write_hw( x264_t *h )
{
    ...
    HwInfo_t * s = (HwInfo_t *)h->hwinfo;
    _H264E_SliceInfo * sliceinfo = &s->H264E_SliceInfo;
    unsigned int tlb_addr = 0;
    unsigned int vpu_status = 0;

    EL("start slice write");
      /* Slice */
    ...
    EL(" tile stuff buffer ... ");
#ifdef USE_BFRAME
      /* FIXME : remain this for only support one forward pred pict, maybe used in future
    ...
    // init in slices_write, before calling current func.
    sliceinfo->first_mby = h->sh.i_first_mb / h->sps->i_mb_width;
    sliceinfo->last_mby = h->sh.i_last_mb / h->sps->i_mb_width;

    // init in x264_slice_header_init
    sliceinfo->deblock = !h->sh.i_disable_deblocking_filter_idc;
    sliceinfo->rotate = 0x0;
    sliceinfo->alpha_c0_offset = h->sh.i_alpha_c0_offset;
    sliceinfo->beta_offset = h->sh.i_beta_offset;

    // cabac init
    sliceinfo->state = &h->cabac.state[0];
    sliceinfo->qp = h->sh.i_qp;
    //sliceinfo->qp = x264_ratecontrol_qp( h ); //h->rc->qpm

    sliceinfo->des_va = s->vdma_config;
    sliceinfo->des_pa = s->vdma_config;
    ...
    EL("H264E_SliceInit");
    H264E_SliceInit(sliceinfo);
    jz_dcache_wb();

    //clear state
    //*(volatile unsigned int *)(dblk_base + 0x70) = 0x0;  //this way maybe not very clean
    //*(volatile unsigned int *)(sde_base + 0x0) = 0x0;
    //*(volatile unsigned int *)(vpu_base + 0x34) = 0x0;
...
/*  EL("config end [state] vdma=%x, sde=%x, vpu=%x\n", read_vpu_reg(gp0_base + 0xC, 0), read_vpu_reg(sde_base + 0x0, 0), read_vpu_reg(vpu_base + 0x34, 0)); */
    ...
    bs_len = read_vpu_reg(sde_base + 0x38, 0x0) & 0xFFFFFF;
#ifdef JZC_PMON_P0
    PMON_OFF(hw);
#endif
    EL("bs_len = 0x%x", bs_len);
    jz_dcache_wb();

    if ( h->out.bs.i_left == 32 ){
        memcpy(h->out.bs.p, s->bs_ptr, bs_len);
        h->out.bs.p += bs_len;
        EL("[ bs ] fast copy complete", h->out.bs.i_left);
    } else {
        int length = bs_len;
        volatile unsigned char * bit_ptr = s->bs_ptr;
        while( length-- )
            bs_write(&h->out.bs, 8, *bit_ptr++);
        bs_rbsp_trailing(&h->out.bs);
    }

#ifdef CRC_CHECK_n
    bs_total_len += bs_len;
    bs_crc = crc(s->bs_ptr, bs_len, bs_crc);
    EL("bs crc = %d", bs_crc);
    dec_y_crc = crc(s->fb_ptr[0][0], sliceinfo->mb_width*sliceinfo->mb_height*256, dec_y_crc);
    dec_c_crc = crc(s->fb_ptr[0][1], sliceinfo->mb_width*sliceinfo->mb_height*128, dec_c_crc);
#endif
...
static void *x264_slices_write( x264_t *h )
{
    ...
    while( h->sh.i_first_mb <= last_thread_mb )
    {
        ...
        if( x264_stack_align( x264_slice_write, h ) )
            return (void *)-1;
#else
        if( x264_stack_align( x264_slice_write_hw, h ) )
            return (void *)-1;
                             ...
                             x264_picture_t *pic_out )
{
    ...
    *pp_nal = NULL;
    ...
    pic_in->img.i_csp = h->param.i_csp;
...
#ifdef JZC_PMON_P0
    {
      int mb_num = h->sps->i_mb_width*h->sps->i_mb_height;
      ...
      //cache_save_pmon_val = 0; cache_save_pmon_val_ex = 0;

      //printf("D: %d; I:%d\n", get_pmon_val()/mb_num, get_pmon_val_ex()/mb_num);
      //pmon_reset();
...
static void zigzag_scan_4x4_hw( int16_t level[16], const int16_t dct[16] )
{
    ...
    S32SFL(xr9,xr6, xr4,xr10,3);
    S32SFL(xr4,xr10,xr7,xr6, 3);
    ...
    S32STD(xr5, level,0x8);
    S32STD(xr7, level,0xc);
...
static inline void zigzag_scan_2x2_dc_hw( int16_t level[4], const int16_t dct[4] )
{
#if 0
    level[0] = dct[0];
    level[1] = dct[1];
    ...
    S32LDD(xr11, dct, 0x0);
    S32LDD(xr12, dct, 0x4);
    S32STD(xr11, level, 0x0);
    S32STD(xr12, level, 0x4);
...
ALWAYS_INLINE void x264_macroblock_cache_rect4_hw( void *dst, int width, int height, uint32_t val )
{
    int dy;
