...
static int compensate_count_sub = 0;
...
void x264_ratecontrol_start( x264_t *h, int i_force_qp, int overhead )
{
    ...
    const static float weight_coeffs[7][8] = 
    {
        ...
        {1.26, 1.5 , 1.7 , 2.4 , 3.2 , 0.75, 0.52, 0.32},
        {1.35, 1.55, 1.72, 2.5 , 3.25, 0.72, 0.48, 0.3 },
		...
		last_in_frame =  h->i_frame ;
            else
            {
                if ( fbr_bits >= expect_bits * weight_coeffs[weight_index][0] && \
                     fbr_bits <  expect_bits * weight_coeffs[weight_index][1] )
...
#if 0  /* this func's updateing realization moves to x264_ratecontrol_end.  peng */
...
int x264_ratecontrol_end( x264_t *h, int bits )
{
    ...
    /* obtain real encoded bits of the last frame. peng */
    fbr_bits = bits;
