...
#ifdef P1_USE_PADDR
#define MOTION_BASE (0x13250000)
#define MOTION_V_BASE MOTION_BASE
#else 
extern volatile unsigned char *mc_base;
#define MOTION_V_BASE       (int)mc_base
#endif
...
//#define write_reg(reg, val) *(volatile int*)(reg) = (volatile unsigned int)val
...
#define SET_REG1_DSTA(dsta)						\
  ({ write_reg( (MOTION_V_BASE+REG1_DSTA), ((int)dsta) );			\
  })
