...
#else
    fprintf( stderr,
	     "Useage: guess input file height and width, and cut from .. to .. \n"
             "Syntax: guess_size file frome to\n"
             "  --help                  Print this help\n"
             "  -file, <strng>          input yuv file name\n"
             "  -from, <integer>        cut source file from this picture\n"
             "  -to, <integer>          cut source file to this picture\n"
             "\n" );
#endif
}
...
static void xyuv_cut( int from, int to );
...
int main( int argc, char **argv )
{
    ...
    if(argc >= 3){
      ...
      int to = atoi(argv[3]);
      xyuv_cut( from, to );
      printf("cutting form:%d to %d done \n", from, to);
      //printf(" need to move the file new_cut.yuv to a new name \n", from, to);
    }
...
static void xyuv_detect( int *pi_width, int *pi_height )
{
    ...
    *pi_height = 0;
    ...
    for( i = 0; i < xyuv.i_yuv; i++ ) {
        ...
        //fprintf( stderr, " - %s \t", xyuv.yuv[i].name );
...
static int get_frame_total( FILE * f, int i_frame_size )
{
    int i_frame_total = 0;
    fseek( f, 0, SEEK_END );
    uint64_t i_size = ftell( f );
    fseek( f, 0, SEEK_SET );
    i_frame_total = (int)( i_size / i_frame_size );
...
static void xyuv_cut( int from, int to )
{
  ...
  dst_fp = fopen( "new_cut.yuv", "aw" );
  if(dst_fp == NULL)
    printf(" error opening new file ! \n");

  i_frame_size = xyuv.i_width * xyuv.i_height * 3 / 2 ;

  pic = malloc( i_frame_size );

  //i_frame_total = get_frame_total(xyuv.yuv[0].f, i_frame_size);
  i_frame_total = 0;
  fseek( xyuv.yuv[0].f, 0, SEEK_END );
  uint64_t i_size = ftell( xyuv.yuv[0].f );
  fseek( xyuv.yuv[0].f, 0, SEEK_SET );
  i_frame_total = (int)( i_size / i_frame_size );
  ...
  uint64_t seek_from = from * i_frame_size;
  fseek( xyuv.yuv[0].f, seek_from, SEEK_SET );

  uint64_t i_pos;
  i_pos = ftell( xyuv.yuv[0].f );
  //printf(" curr picture pos: %lld \n",i_pos);

  int i_distanct = to - from;
  for (i=0; i<i_distanct; i++){

    if( fread( pic, 1, i_frame_size, xyuv.yuv[0].f ) != i_frame_size ){
      printf(" ---- data read error ! ---- \n");
      break;
    }

    if( fwrite( pic, 1, i_frame_size, dst_fp ) != i_frame_size){
