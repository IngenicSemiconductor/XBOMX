...
#if defined(X264_CRC_CHECK) || defined(X264_CRC_CHECK_FRAME)
#include "soc/crc.h"
char *x264_input_filename;
char *x264_output_filename;
#endif 
...
#ifdef DUMP_MOTION
FILE *dump_motion_fp;
int dump_motion_en = 0;
int dump_motion_fn = 0;
int encoder_fn = 0;
int mb_num = 0;
int step_cnt = 0;
int step_sum = 0;
...
int main( int argc, char **argv )
{
...
#ifdef DUMP_MOTION_
    for(i=1; i<argc-1; i++){
      if ( !strcmp(argv[i], "--dump-motion") ){
	dump_motion_en = 1;
	dump_motion_fn = atoi(argv[i+1]);
	dump_motion_fp = fopen("motion.dump", "w+");
	if(dump_motion_fp == NULL)
	  exit(1);
      }
    }
#endif
    ...
    if(encoder_fn)
      printf("++AVG STEP: %.2f +++++++++++++++++++++++++++++++\n", 
	     (float)step_sum/(float)(mb_num*encoder_fn) );
    if(dump_motion_fp != NULL)
      fclose(dump_motion_fp);
#endif
...
#define H2 if(longhelp==2) printf
    ...
    /* --qp only provides the initial qp, whereas the fbr mode will change the value of qp before the current frame is\
     * encoded so that to achieve relatively constant bitrate.*/
    H0( "      --fbr  <integer>        Use FBR rate control mode which is compatible with CQP\n");     
    H0( "\n" );
}
...
static struct option long_options[] =
{
    ...
    { "bug-f",   required_argument, NULL, 0 },
    { "bug-mbx",   required_argument, NULL, 0 },
    { "bug-mby",   required_argument, NULL, 0 },
    { "eyer-debug",   required_argument, NULL, 0 },
    { "eyer-en",   required_argument, NULL, 0 },
    { "eyer-random",   required_argument, NULL, 0 },
    { "eyer-wave",   required_argument, NULL, 0 },
    { "dump-motion", required_argument, NULL, 0 },
    { "fbr",         required_argument, NULL, 0 },
    {0, 0, 0, 0}
};
...
#if defined(X264_CRC_CHECK) || defined(X264_CRC_CHECK_FRAME)
    //x264_input_filename = input_filename;
    //x264_output_filename = output_filename;
#endif
...
static int  Encode( x264_param_t *param, cli_opt_t *opt )
{
...
#ifdef X264_CRC_CHECK_FRAME
    static unsigned short int crc_code2 = 0;//FIXME: crc function should return unsigned short
    static int64_t file_last_index = 0;
#endif
...
#if defined(JZC_PMON_P0) || defined(X264_CRC_CHECK_FRAME)
#else
	//FIXME: it seems very slow;gjwang
...
#ifdef X264_CRC_CHECK_FRAME
	{
	  FILE *coded_f;
	  int cur_len = 0;
	  coded_f = fopen( x264_output_filename, "r+b" );
	  if (!coded_f){
	    printf(" error while open coded file; crc_check error! \n");
	  } else {
	    fflush(coded_f);	    
	    fseek( coded_f, 0, SEEK_END );
	    cur_len = ftell( coded_f ) - file_last_index;
	      
	    fseek( coded_f, -cur_len, SEEK_END );
	    uint8_t *crc_p = x264_malloc( cur_len + 0x100 );

	    if(NULL == crc_p){
	      printf("x264_malloc failed!\n");
	      exit(1);
	    }

	    fread(crc_p, 1, cur_len, coded_f);

	    crc_code2 = crc(crc_p, cur_len, crc_code2);
	    //dump_bs(crc_p, cur_len);

	    file_last_index += cur_len;

	    //printf("len = %d, cur_len = %d\n", len, cur_len);

	    //printf("frame_num: %d, crc:0x%x\n", h->i_frame, crc_code2);
	    printf("frame_num = %d, crc = 0x%04x\n", i_frame_output, (unsigned short)crc_code2);
	    
	    fclose(coded_f);
	    x264_free(crc_p);
	  }
	}
#endif //X264_CRC_CHECK
    }
...
#ifdef X264_CRC_CHECK_FRAME
    {
	    ...
	    printf("%s: frame_num = %d, crc = 0x%04x\n", 
		   x264_input_filename, i_frame_output, (unsigned short)crc_code2);
	    
	    fclose(coded_f);
	    x264_free(crc_p);
	}
    }
#endif //X264_CRC_CHECK_FRAME


#ifdef X264_CRC_CHECK
    {
      FILE *coded_f;
      int len = 0;
      coded_f = fopen( x264_output_filename, "r+b" );
	...
	fseek( coded_f, 0, SEEK_SET );
	uint8_t *crc_p = x264_malloc( len + 0x100 );
	...
	fread(crc_p, 1, len, coded_f);
	short crc_code = crc(crc_p, len, 0);
	printf("name:%s: crc:0x%4x\n", x264_input_filename, (unsigned short)crc_code);
