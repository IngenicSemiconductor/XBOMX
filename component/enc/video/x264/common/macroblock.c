...
#include "soc/config_jz_soc.h"
#ifdef JZ_HW_ACCEL
#include "soc/jz4760e_2ddma_hw.h"
#include "soc/tcsm1.h"
#include "soc/sram.h"
extern TCSM1 *v_tcsm1;   //v: Virtual addr
extern SRAM *v_sram;   //v: Virtual addr

extern TCSM1 *tcsm1_phy;
extern SRAM *sram_phy;
#endif
...
void x264_mb_predict_mv( x264_t *h, int i_list, int idx, int i_width, int16_t mvp[2] )
{
...
#ifdef ENTROPY_PIPE
    const int i_ref= entropy_var_ptr->cache.ref[i_list][i8];
    int     i_refa = entropy_var_ptr->cache.ref[i_list][i8 - 1];
    int16_t *mv_a  = entropy_var_ptr->cache.mv[i_list][i8 - 1];
    int     i_refb = entropy_var_ptr->cache.ref[i_list][i8 - 8];
    int16_t *mv_b  = entropy_var_ptr->cache.mv[i_list][i8 - 8];
    int     i_refc = entropy_var_ptr->cache.ref[i_list][i8 - 8 + i_width];
    int16_t *mv_c  = entropy_var_ptr->cache.mv[i_list][i8 - 8 + i_width];
#else
    ...
    {
#ifdef ENTROPY_PIPE
        i_refc = entropy_var_ptr->cache.ref[i_list][i8 - 8 - 1];
        mv_c   = entropy_var_ptr->cache.mv[i_list][i8 - 8 - 1];
#else
...
void x264_mb_predict_mv_16x16( x264_t *h, int i_list, int i_ref, int16_t mvp[2] )
{
    ...
    //int i_neighbour = h->mb.i_neighbour;
    int i_neighbour = v_tcsm1->mb.i_neighbour;

    if(i_neighbour & MB_LEFT)
    {
        M32(mv_a) = M32(v_tcsm1->left_mv);
    }else
    {
        M32(mv_a) = 0;
    }
    ...
    {
      //CP32(mv_b, entropy_var_ptr->cache.mv[i_list][X264_SCAN8_0 - 8]);
      CP32( mv_b, &v_tcsm1->top_line_mv[v_tcsm1->mb.i_mb_x]);
    }else
    {
      M32(mv_b) = 0;
    }
    ...
    {
      //CP32(mv_c, entropy_var_ptr->cache.mv[i_list][X264_SCAN8_0 - 8 + 4]);
        CP32(mv_c, &v_tcsm1->top_line_mv[v_tcsm1->mb.i_mb_x + 1]);
    }else if(i_neighbour & MB_TOPLEFT)
    {
        //CP32(mv_c, entropy_var_ptr->cache.mv[i_list][X264_SCAN8_0 - 8 - 1]);
        //printf("%d, %d****\n", mv_c[0], mv_c[1]);
        CP32(mv_c, v_tcsm1->topright_mv);
	//printf("%d, %d\n", mv_c[0], mv_c[1]);
    }else
    {
        M32(mv_c) = 0;
    ...
    if( i_refc == -2 )
    {
        i_refc = h->mb.cache.ref[i_list][X264_SCAN8_0 - 8 - 1];//i_neighbour & MB_TOPLEFT
        mv_c   = entropy_var_ptr->cache.mv[i_list][X264_SCAN8_0 - 8 - 1];
...
#if 0
void x264_mb_predict_mv_16x16_hw( int16_t mvp[2] )
{
    TCSM1 *tcsm1_ptr = v_tcsm1;

    int16_t mv_a[2];

    int i_neighbour = tcsm1_ptr->mb.i_neighbour;
    ...
    if(i_neighbour & MB_TOP)
    {
      CP32( mv_b, &tcsm1_ptr->top_line_mv[tcsm1_ptr->mb.i_mb_x]);
    }else
    {
      M32(mv_b) = 0;
    }
    ...
    if(i_neighbour & MB_TOPRIGHT)
    {
        CP32(mv_c, &tcsm1_ptr->top_line_mv[tcsm1_ptr->mb.i_mb_x + 1]);
    }else if(i_neighbour & MB_TOPLEFT)
    {
...
void x264_mb_predict_mv_pskip( x264_t *h, int16_t mv[2] )
{
    ...
    //CP32(entropy_var_ptr->mvp, mv);
    //CP32(v_tcsm1->mb.mvp, mv);
    CP32(v_tcsm1->mv_var.mvp, mv);
    //int i_neighbour = h->mb.i_neighbour;
    int i_neighbour = v_tcsm1->mb.i_neighbour;
...
#ifdef ENTROPY_PIPEed
    const int mvx   = x264_clip3( entropy_var_ptr->cache.mv[0][i8][0], h->mb.mv_min[0], h->mb.mv_max[0] ) + 4*4*x;
    int       mvy   = x264_clip3( entropy_var_ptr->cache.mv[0][i8][1], h->mb.mv_min[1], h->mb.mv_max[1] ) + 4*4*y;
#else
...
#if 0
void ALWAYS_INLINE x264_macroblock_load_raw_data( int i_mb_x, int i_mb_y)
{
    TCSM1 *tcsm1_ptr = v_tcsm1;
    ...
    unsigned char *raw_data_y_ptr = v_sram->raw_data[tcsm1_ptr->gp2_pipe_index].y;    
    unsigned char *raw_data_y_phy_ptr = sram_phy->raw_data[tcsm1_ptr->gp2_pipe_index].y;    


    DMA_TASK_CHAIN *gp2_task_chain_ptr = &tcsm1_ptr->gp2_task_chain_array[tcsm1_ptr->gp2_pipe_index];
    DMA_TASK_CHAIN *gp2_task_chain_phy_ptr = &tcsm1_phy->gp2_task_chain_array[tcsm1_ptr->gp2_pipe_index];
    ...
    unsigned char *raw_data_uv_ptr = v_sram->raw_data[tcsm1_ptr->gp2_pipe_index].uv;    
    unsigned char *raw_data_uv_phy_ptr = sram_phy->raw_data[tcsm1_ptr->gp2_pipe_index].uv;    
...
void x264_macroblock_cache_load_nnz( x264_t *h, int i_mb_x, int i_mb_y )
{
    ...
    {
        /* load non_zero_count */
        CP32( &nnz[x264_scan8[0] - 8], &h->mb.non_zero_count[i_top_xy][12] );
        /* shift because x264_scan8[16] is misaligned */
        M32( &nnz[x264_scan8[16+0] - 9] ) = M16( &h->mb.non_zero_count[i_top_xy][18] ) << 8;
        M32( &nnz[x264_scan8[16+4] - 9] ) = M16( &h->mb.non_zero_count[i_top_xy][22] ) << 8;
    }
    else
    {
        /* load non_zero_count */
        M32( &nnz[x264_scan8[   0] - 8] ) = 0x80808080U;
        M32( &nnz[x264_scan8[16+0] - 9] ) = 0x80808080U;
        M32( &nnz[x264_scan8[16+4] - 9] ) = 0x80808080U;
    }
    ...
    if( i_mb_x > 0 && i_mb_xy > h->sh.i_first_mb )
    {
        ...
        /* load non_zero_count */
        nnz[x264_scan8[0 ] - 1] = h->mb.non_zero_count[i_left_xy][3];
        nnz[x264_scan8[2 ] - 1] = h->mb.non_zero_count[i_left_xy][7];
        nnz[x264_scan8[8 ] - 1] = h->mb.non_zero_count[i_left_xy][11];
        nnz[x264_scan8[10] - 1] = h->mb.non_zero_count[i_left_xy][15];

        nnz[x264_scan8[16+0] - 1] = h->mb.non_zero_count[i_left_xy][16+1];
        nnz[x264_scan8[16+2] - 1] = h->mb.non_zero_count[i_left_xy][16+3];

        nnz[x264_scan8[16+4+0] - 1] = h->mb.non_zero_count[i_left_xy][16+4+1];
        nnz[x264_scan8[16+4+2] - 1] = h->mb.non_zero_count[i_left_xy][16+4+3];
    }
    else
    {
        /* load non_zero_count */
        ...
        nnz[x264_scan8[8 ] - 1] =
        nnz[x264_scan8[10] - 1] =
        nnz[x264_scan8[16+0] - 1] =
        nnz[x264_scan8[16+2] - 1] =
        nnz[x264_scan8[16+4+0] - 1] =
        nnz[x264_scan8[16+4+2] - 1] = 0x80;
    }
...
#if 1
#else
void x264_macroblock_cache_load_no_nnz( x264_t *h, int i_mb_x, int i_mb_y )
{
    int i_mb_stride = v_tcsm1->slice_var.i_mb_stride;

    int i_mb_width = v_tcsm1->slice_var.i_mb_width;
    int i_first_mb = v_tcsm1->slice_var.i_first_mb;
    ...
    v_tcsm1->mb.i_mb_x = i_mb_x;
    v_tcsm1->mb.i_mb_y = i_mb_y;
    v_tcsm1->mb.i_mb_xy = i_mb_xy;
    ...
    {
        if(i_mb_x + 1 == h->sps->i_mb_width - 1)
	{
	  CP32(v_tcsm1->topright_mv, &v_tcsm1->top_line_mv[v_tcsm1->mb.i_mb_x]);
        }
    ...
    v_tcsm1->mb.i_neighbour = i_neighbour;
}
#endif
...
#if 0
void ALWAYS_INLINE x264_macroblock_store_pic_vmau( x264_t *h, int i )
{
    int w = i ? 8 : 16;
    int i_stride = h->fdec->i_stride[!!i];
    int i_stride2 = i_stride << h->mb.b_interlaced;
    
    //ENTROPY_VAR *last_entropy_var_ptr =&entropy_var[v_tcsm1->pipe_index];

    int i_mb_x = entropy_var_ptr->i_mb_x;
    int i_mb_y = entropy_var_ptr->i_mb_y;
    
    //printf("i_mb_x = %d, i_mb_y = %d, i_mb_xy = %d\n", i_mb_x, i_mb_y, last_entropy_var_ptr->i_mb_xy);
...
#if 0
void ALWAYS_INLINE x264_macroblock_store_pic_vmau_y( x264_t *h, int i )
{
    int w = i ? 8 : 16;
    int i_stride = h->fdec->i_stride[!!i];
    int i_stride2 = i_stride << h->mb.b_interlaced;
    ...
    ENTROPY_VAR const * const last_entropy_var_ptr =&entropy_var[!v_tcsm1->pipe_index];
    volatile VMAU_VAR const * const vmau_var = &v_tcsm1->vmau_var[!v_tcsm1->pipe_index];

    int i_mb_x = last_entropy_var_ptr->i_mb_x;
    int i_mb_y = last_entropy_var_ptr->i_mb_y;
    ...
    h->mc.copy[i?PIXEL_8x8:PIXEL_16x16](
        &h->fdec->plane[i][i_pix_offset], i_stride2,
        vmau_var->dec_dst_y, 16, w );
#endif

}


void ALWAYS_INLINE x264_macroblock_store_pic_vmau_u( x264_t *h, int i )
{
    int w = i ? 8 : 16;
    int i_stride = h->fdec->i_stride[!!i];
    int i_stride2 = i_stride << h->mb.b_interlaced;
    ...
    ENTROPY_VAR *last_entropy_var_ptr =&entropy_var[!v_tcsm1->pipe_index];
    volatile VMAU_VAR * const vmau_var = &v_tcsm1->vmau_var[!v_tcsm1->pipe_index];
    ...
    h->mc.copy[i?PIXEL_8x8:PIXEL_16x16](
        &h->fdec->plane[i][i_pix_offset], i_stride2,
        vmau_var->dec_dst_uv, 16, w );
}

void ALWAYS_INLINE x264_macroblock_store_pic_vmau_v( x264_t *h, int i )
{
    int w = i ? 8 : 16;
    int i_stride = h->fdec->i_stride[!!i];
    int i_stride2 = i_stride << h->mb.b_interlaced;
    ...
    h->mc.copy[i?PIXEL_8x8:PIXEL_16x16](
        &h->fdec->plane[i][i_pix_offset], i_stride2,
        vmau_var->dec_dst_uv + 8, 16, w );
}

#endif

#ifdef ENTROPY_PIPEed
void ALWAYS_INLINE x264_macroblock_store_pic( x264_t *h, int i )
{
    int w = i ? 8 : 16;
    int i_stride = h->fdec->i_stride[!!i];
    int i_stride2 = i_stride << h->mb.b_interlaced;
...
static void ALWAYS_INLINE x264_macroblock_store_pic( x264_t *h, int i )
{
    ...
    //dump_vmau_dec_rst(h->mb.pic.p_fdec[i]);

}
#endif

#if 0
void x264_macroblock_cache_save_no_nnz_mv( x264_t *h )
{
    const int i_mb_xy = entropy_var_ptr->i_mb_xy;
    const int i_mb_type = x264_mb_type_fix[entropy_var_ptr->i_type];

    int b_transform_8x8 = entropy_var_ptr->b_transform_8x8;

    const int i_cbp_luma = entropy_var_ptr->i_cbp_luma;
    const int i_cbp_chroma = entropy_var_ptr->i_cbp_chroma;

#ifdef ENTROPY_PIPE
    const int s8x8 = entropy_var_ptr->i_b8_stride;
    const int s4x4 = entropy_var_ptr->i_b4_stride;
    const int i_mb_4x4 = entropy_var_ptr->i_b4_xy;
    const int i_mb_8x8 = entropy_var_ptr->i_b8_xy;
#else
    const int s8x8 = h->mb.i_b8_stride;
    const int s4x4 = h->mb.i_b4_stride;
    ...
    if( i_mb_type == I_4x4 )
    {
        CP32( &intra4x4_pred_mode[0], &entropy_var_ptr->cache.intra4x4_pred_mode[x264_scan8[10]] );
        M32( &intra4x4_pred_mode[4] ) = pack8to32(entropy_var_ptr->cache.intra4x4_pred_mode[x264_scan8[5] ],
                                                  entropy_var_ptr->cache.intra4x4_pred_mode[x264_scan8[7] ],
                                             entropy_var_ptr->cache.intra4x4_pred_mode[x264_scan8[13] ], 0);
    }
...
#ifdef ENTROPY_PIPE
    {
        uint8_t *nnz = &entropy_var_ptr->cache.non_zero_count[0];
        /* save non zero count */
	CP32( &non_zero_count[0*4], &nnz[x264_scan8[0]+0*8] );
        CP32( &non_zero_count[1*4], &nnz[x264_scan8[0]+1*8] );
        CP32( &non_zero_count[2*4], &nnz[x264_scan8[0]+2*8] );
        CP32( &non_zero_count[3*4], &nnz[x264_scan8[0]+3*8] );
        M16( &non_zero_count[16+0*2] ) = M32( &nnz[x264_scan8[16+0*2]-1] ) >> 8;
        M16( &non_zero_count[16+1*2] ) = M32( &nnz[x264_scan8[16+1*2]-1] ) >> 8;
        M16( &non_zero_count[16+2*2] ) = M32( &nnz[x264_scan8[16+2*2]-1] ) >> 8;
        M16( &non_zero_count[16+3*2] ) = M32( &nnz[x264_scan8[16+3*2]-1] ) >> 8;

        if( entropy_var_ptr->i_type != I_16x16 && entropy_var_ptr->i_cbp_luma == 0 && 
	    entropy_var_ptr->i_cbp_chroma == 0 )
            entropy_var_ptr->i_qp = h->mb.i_last_qp;
        h->mb.qp[i_mb_xy] = entropy_var_ptr->i_qp;
        h->mb.i_last_dqp = entropy_var_ptr->i_qp - h->mb.i_last_qp;
        h->mb.i_last_qp = entropy_var_ptr->i_qp;
    }
#else
    {
        /* save non zero count */
    ...
    if( i_cbp_luma == 0 && entropy_var_ptr->i_type != I_8x8 )
        b_transform_8x8 = 0;
    h->mb.mb_transform_size[i_mb_xy] = b_transform_8x8;
    ...
    if( h->sh.i_type != SLICE_TYPE_I )
    {
        ...
        {
#ifdef ENTROPY_PIPE
	    int8_t *ref = &entropy_var_ptr->cache.ref[0];
            h->mb.ref[0][i_mb_8x8+0+0*s8x8] = ref[x264_scan8[0]];
            h->mb.ref[0][i_mb_8x8+1+0*s8x8] = ref[x264_scan8[4]];
            h->mb.ref[0][i_mb_8x8+0+1*s8x8] = ref[x264_scan8[8]];
            h->mb.ref[0][i_mb_8x8+1+1*s8x8] = ref[x264_scan8[12]];
#if 0
            for( y = 0; y < 4; y++ )
            {
                CP64( h->mb.mv[0][i_mb_4x4+y*s4x4+0], entropy_var_ptr->cache.mv[0][x264_scan8[0]+8*y+0] );
                CP64( h->mb.mv[0][i_mb_4x4+y*s4x4+2], entropy_var_ptr->cache.mv[0][x264_scan8[0]+8*y+2] );
            }
#endif
#else
...
#if 0
void x264_macroblock_cache_save_mv(x264_t *h, ENTROPY_VAR *entropy_var_ptr)
{
    const int i_mb_xy = entropy_var_ptr->i_mb_xy;
    const int i_mb_type = x264_mb_type_fix[entropy_var_ptr->i_type];
...
inline void x264_macroblock_cache_save_nnz( x264_t *h, int i_mb_xy)
{
    ...
    uint8_t *nnz = &h->mb.cache.non_zero_count[0];
    /* save non zero count */
    CP32( &non_zero_count[0*4], &nnz[x264_scan8[0]+0*8] );
    CP32( &non_zero_count[1*4], &nnz[x264_scan8[0]+1*8] );
    CP32( &non_zero_count[2*4], &nnz[x264_scan8[0]+2*8] );
