...
static inline void bs_write( bs_t *s, int i_count, uint32_t i_bits )
{
 ...
 /*  roughly, this slight changing may decrease 1 pmon per call.*/
  ...
  //START(bs);
    ...
    {
        //i_count -= s->i_left;
        register int32_t tmp = s->i_left + i_count;
        register int32_t tmp2 = i_count - tmp;
	//s->cur_bits = (s->cur_bits << s->i_left) | (i_bits >> i_count);
        tmp = (s->cur_bits << tmp ) | (i_bits >> tmp2 );
        //M32( s->p ) = endian_fix( s->cur_bits );
        M32( s->p ) = endian_fix( tmp );
        s->p += 4;
        s->cur_bits = i_bits;               /* i_left will ensure the final correctness.  */
        s->i_left = 32 - tmp2;
        //s->i_left = 32 - i_count;
    }
