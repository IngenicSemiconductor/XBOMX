 ...
 *
 * $Id: layer3.c,v 1.1 2010/12/30 02:47:26 hpwang Exp $
 */
...
//#define JZC_CRC_VER_MAD
#ifdef JZC_CRC_VERMAD
...
# endif
  ...
  PMON_CLEAR(mp3);
#endif
#ifdef JZC_CRC_VER_MAD
  {
    int i=0;
    signed long (*tfrm);
    tfrm = frame->sbsample[0];
    for (i = 0; i < 36*32; i++){
      crc_code = crc(&(tfrm[i]), 4, crc_code);
    }
    printf("rota_crc: 0x%x\n", crc_code);
  }
#endif
