/* ***** BEGIN LICENSE BLOCK *****   
 * Source last modified: $Id: radec_fft.c,v 1.1.1.1 2007/12/07 08:11:48 zpxu Exp $ 
 * 
...
/*    
/*		1. Mapping of tables to appropriate sections
...
#define swapcplx(p0,p1) \
	t = p0;  p0 = p1; p1 = t;t1 = *(&(p0)+1); *(&(p0)+1) = *(&(p1)+1); *(&(p1)+1) = t1;
...
static void BitReverse(int *inout, int tabidx)
{
	...
	while ((a = *tab++) != 0) {
        b = *tab++;
	swapcplx(part0[4*a], part0[4*b]);	/* 0xxx0 <-> 0yyy0 */
        swapcplx(part0[4*a+2], part1[4*b]);	/* 0xxx1 <-> 1yyy0 */
        swapcplx(part1[4*a], part0[4*b+2]);	/* 1xxx0 <-> 0yyy1 */
