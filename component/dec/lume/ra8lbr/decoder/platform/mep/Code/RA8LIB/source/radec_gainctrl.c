/* ***** BEGIN LICENSE BLOCK *****   
 * Source last modified: $Id: radec_gainctrl.c,v 1.1.1.1 2007/12/07 08:11:48 zpxu Exp $ 
 * 
 ...
 **************************************************************************************/
int DecodeGainInfo(Gecko2Info *gi,SBitStreamInfo *bsi, SGainCInfo *gainc, int availbits)
{
	int i, nbits, code;
	...
	nbits = 0;
	do {
		/*Number of attacks must not exceed 8*/
		if(nbits > 8)
			return -2;		
		if (availbits <= 0)
			return -1;
 ...
 **************************************************************************************/
void CopyGainInfo(SGainCInfo *gaincDest, SGainCInfo *gaincSource)
{
	int nBytes = sizeof(SGainCInfo);
 ...
 **************************************************************************************/
static void CalcGainChanges(int *exgain, SGainCInfo *gainc0, SGainCInfo *gainc1)
{
 ...
 **************************************************************************************/
static void InterpolatePCM(int tabidx, int *exgain, int *buf, int maxGain, short *pcm, int nChans, int fbitsPCM, int fbitsOver,unsigned char *ErrParam)
{
	int in, npsamps, part;
 ...
 *                the output buffer (pcm) for stereo interleaving
 **************************************************************************************/
void DecWindowWithAttacks(int tabidx, int *buf, short *pcm, int nChans, SGainCInfo *gainc0, SGainCInfo *gainc1, int xbits[2],unsigned char *ErrParam)
{
	...
	/*ErrParam added by Deepak for error handling*/
	InterpolatePCM(tabidx, exgain, buf, gainc0->maxExGain, pcm, nChans, fbitsPCM, fbitsOver,ErrParam);
	if(*ErrParam == 1)
		return;
