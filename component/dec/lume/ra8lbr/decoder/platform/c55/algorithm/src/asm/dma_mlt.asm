; ***** BEGIN LICENSE BLOCK *****   
; Source last modified: $Id: dma_mlt.asm,v 1.1.1.1 2007/12/07 08:11:44 zpxu Exp $ 
;  
...
;-----------------------------------------------------------------------------------------------------------
; 	File 		:	dma_mlt.asm 
;   Description : C55X assembly routines for functions present in mlt.c
;					 WITH DMA support enabled
;	
;------------------------------------------------------------------------------------------------------------
;
	.if CONST_TABLE_DMA	
	; dma support for constant tables enabled
	...
	.ref _ACPY2_wait
	.ref _ACPY2_start
	.ref _RA_TNI_DMA_PARAMS		; parameters for dma movement
...
; Input : AR0 -> zbuf (long*)
;		: AR1 -> zfft(long*)  
; 		: AR2 -> fixcos4tab ( the correct one)
; 		: AR3 -> fixsin4tab ( the correct one)
;		: T0 -> nsamples (short)
;
;Returns : void
;
...
;  Input : AR0 -> fft (long*)
;		: AR1 -> mlt(long*)     
;		: XAR2 -> fixcos1tab (the correct one)
;		: T0 -> nsamples (short)
;
; Returns : void
;
...
_RA_TNI_PostMultiply:
	...
	||
	if( AC3 > 0 ) execute(AD_Unit)  ; if positive overflow due to shifting;to eliminate CPU_30
	...
	||
	if(AC3 < 0 ) execute(AD_Unit) 	; if negative overflow;to eliminate CPU_30
...
_AR6 .set 0016h		; memory mapped reg
	...
	||
	if( AC2 > 0 ) execute(AD_Unit)  ; if positive overflow due to shifting;to eliminate CPU_30
		AC1 = dbl(*(#possat) )	   ; positively saturate AC1
	
	AC2 = AC2 + #1 	; to check for neg. underflow
    nop;added to avoid silicon exception causing remark 5505
	
	if(AC2 < 0 ) execute(AD_Unit) 	; if negative overflow;to eliminate CPU_30
	...
	||
	XCDP = XAR2						
	; cdp = nop;added to avoid silicon exception causing remark 5505cr_h for next pass
	; initialze to cosptr
...
;  Input : AR0 -> zbuf (long*)
;		: AR1 -> pcm (long*)
;		: XAR6 -> fixwindowtab (the correct one )
;		: T0 -> nsamples (short)
;
; Returns : void
;
...
_RA_TNI_IMLTNoOverlap:
	...
	*SP(#6) = T0
	...
	; SP(4) -> XAR2 (buf)
	; SP(6) -> T0   (nsamples)
	; SP(8) -> XAR3 (bitstrmObj)

	; wait on channel 0, for DMA tables to be paged in
	XAR0 = dbl(*AR3(#(RA_TNI_Obj.dmaChannel_0)) )
	; XAR0 -> dmaChannel_0
	bit(ST2, #15) = #1
    ; clear the ARMS bit for client
	call _ACPY2_wait
	; wait for cos4sin4 tables
     
    ; bring in twiddle tabs on channel 0
    XAR0 = dbl(*SP(#8) ) 
    AC0 = dbl(*AR0(#(RA_TNI_Obj.twidtab_extmem) ))
    ; AC0 (src_address) = objPtr->twidtab_extmem          
    AC2 = dbl(*AR0(#(RA_TNI_Obj.twidtabsize))) 
    ; AC2 (cnt) = objPtr->twidtabsize
    AC1 = dbl(*AR0(#(RA_TNI_Obj.twidtab) )) 
    ; AC1 (dst_address) = objPtr->twidtab
  
    T1 = *AR0(#( RA_TNI_Obj.currChannel))
    ; T1 = 0, if this is first time 
 
    XAR0 = dbl(*AR0(#(RA_TNI_Obj.dmaChannel_0) )) 
    ; XAR0 (channel) = objPtr->dmaChannel_0
   ...
   ; clear the ARMS bit for client
    ...
    if( T1 == 0 ) call _ACPY2_start
    ; Bring in the twiddle tables only the first time
	...
	T0 = *SP(#6)
	XAR1 = dbl(*SP(#2))
    XAR2 = dbl(*AR3(#(RA_TNI_Obj.fixcos4tab)))
	XAR3 = dbl(*AR3(#(RA_TNI_Obj.fixsin4tab)))	
	; XAR0 -> mlt
	; XAR1 -> pcm
	; XAR2 -> fixcos4tab
	; XAR3 -> fixsin4tab
	...
	XAR3 = dbl(*SP(#8) )
    ; XAR3 -> bitstrmPtr
    T1 = *AR3(#( RA_TNI_Obj.currChannel))
    ; T1 = 0, if this is first time 

	XAR0 = dbl(*AR3(#(RA_TNI_Obj.dmaChannel_0)) )
	; XAR0 -> dmaChannel_0
	bit(ST2, #15) = #1
    ; set the ARMS bit for client
	if( T1 == 0 ) call _ACPY2_wait
	; twiddle tabs will be paged in only the first time
	
	;page in cos1 tables on channel 0
	XAR0 = dbl(*SP(#8) ) 
    AC0 = dbl(*AR0(#(RA_TNI_Obj.fixcos1tab_extmem) ))
    ; AC0 (src_address) = objPtr->fixcos1tab_extmem          
    AC2 = dbl(*AR0(#(RA_TNI_Obj.fixcos1size))) 
    ; AC2 (cnt) = objPtr->fixcos1size
    AC1 = dbl(*AR0(#(RA_TNI_Obj.fixcos1tab) )) 
    ; AC1 (dst_address) = objPtr->fixcos1tab
    ...
    if( T1 == 0 ) call _ACPY2_start
    ; Bring in the cos1 tables only the first time
    
	
	XAR6 = dbl(*SP(#8) )
	XAR0 = dbl(*SP(#2))
	T0 = *SP(#6)        
	XAR6 = dbl(*AR6(#(RA_TNI_Obj.twidtab)))
	bit(ST2, #15) = #0
    ; reset the ARMS bit
    ||
	; XAR0 -> pcm
	...
	; wait on channel 0, for cos1 tables to be paged in
	XAR3 = dbl(*SP(#8) )
	XAR0 = dbl(*AR3(#(RA_TNI_Obj.dmaChannel_0)) )
	; XAR0 -> dmaChannel_0
    T1 = *AR3(#( RA_TNI_Obj.currChannel))
    ...
    ; clear the ARMS bit for client
	if( T1 == 0 ) call _ACPY2_wait
	; cos1 tabs will be paged in only the first time
    																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																		
	; bring in fixwindow tabs on channel 0
    XAR0 = dbl(*SP(#8) ) 
    AC0 = dbl(*AR0(#(RA_TNI_Obj.fixwindow_tab_extmem) ))
    ; AC0 (src_address) = objPtr->fixwindow_tab_extmem          
    AC2 = dbl(*AR0(#(RA_TNI_Obj.fixwindowsize))) 
    ; AC2 (cnt) = objPtr->fixwindowsize
    AC1 = dbl(*AR0(#(RA_TNI_Obj.fixwindow_tab) )) 
    ; AC1 (dst_address) = objPtr->twidtab
    XAR0 = dbl(*AR0(#(RA_TNI_Obj.dmaChannel_0) )) 
    ; XAR0 (channel) = objPtr->dmaChannel_0    
	...
	.if WORKING_BUF_DMA
	 ...
	 ; page in overlap[0] or [1] depending on current channel being processed
	 ; For mono stream -> page in overlap[0]
	 ...
	 T0 = *AR0( #(RA_TNI_Obj.nsamples) )
	 nop
	 nop;added to avoid silicon exception causing remark 5505
	 if(T1 == 0 ) execute(AD_Unit)
	 	AC1 = dbl(*AR0(#(RA_TNI_Obj.overlap)))
	 ; if channel 0	
	 ; AC1 (dst_address) = objPtr->overlap[0]
	 nop
	 nop;added to avoid silicon exception causing remark 5505
	 if( T1 != 0 ) execute(AD_Unit)
	 	AC1 = dbl(*AR0(#(RA_TNI_Obj.overlap+2)))
	 ; if channel 0	
	 ; AC1 (dst_address) = objPtr->overlap[1]	
	 
	 XAR0 = dbl(*AR0(#(RA_TNI_Obj.dmaChannel_1) )) 
	 ; XAR0 (channel) -> objPtr->dmaChannel_1
	 
	 AR1 = AR1 + #(RA_TNI_Obj.overlap_extmem) 
	 ; XAR1 = objPtr + overlap_extmem[0]
	 nop;added to avoid silicon exception causing remark 5505
	 if( T1 != 0) execute (AD_Unit)        ; if ( channel no != 0 )
	 || AR1 = AR1 + #2                     ;   XAR1 = objPtr + overlap_extmem[1]
	 
	 AC0 = dbl(*AR1)
	 ; AC0 (src_address) = objPtr->overlap_extmem[0or1]
	 ||
   	 AC1 = AC1 << #1
   	 ; byte address
	 ...
	 ;-----------------------------------------------------------
	 ; If right channel is being processed, check to see if it is
	 ; safe to use working buffer 1 for temp buffer
	 ; wait on channel 2 
	 XAR3 = dbl(*SP(#8) )
	 ;XAR3-> objPtr
	 ...
	 XAR0 = dbl(*AR3(#(RA_TNI_Obj.dmaChannel_2)) )
	...
	XAR1 = dbl(*SP(#4) )                        
	T0 = *SP(#6)
	XAR2 = dbl(*AR2(#(RA_TNI_Obj.fixcos1tab)))
	; XAR0 -> pcm
	; XAR1 -> buf       
	; XAR2 -> fixcos1tab
	; T0 -> nsamples    
	...
	; wait on channel 1, for DMA tables to be paged in
	XAR3 = dbl(*SP(#8) )
	XAR0 = dbl(*AR3(#(RA_TNI_Obj.dmaChannel_0)) )
	; XAR0 -> dmaChannel_0
	bit(ST2, #15) = #1
    ; set the ARMS bit for client
    call _ACPY2_wait
	; wait for fixwin tabs

	XAR6 = dbl(*SP(#8) )	
	XAR0 = dbl(*SP(#4) )
	XAR1 = dbl(*SP(#2) )
	T0 = *SP(#6)
	XAR6 = dbl(*AR6(#(RA_TNI_Obj.fixwindow_tab)))
    ...
    ; bring in tables for PreMultiply for the next iteration on channel 0
    XAR0 = dbl(*SP(#8) ) 
    AC0 = dbl(*AR0(#(RA_TNI_Obj.fixcos4_sin4tab_extmem) ))
    ; AC0 (src_address) = objPtr->fixcos4_sin4tab_extmem          
    AC2 = dbl(*AR0(#(RA_TNI_Obj.fixcos4sin4size))) 
    ; AC0 (cnt) = objPtr->fixcos4sin4size
    AC1 = dbl(*AR0(#(RA_TNI_Obj.fixcos4tab) )) 
    ; AC1 (dst_address) = objPtr->fixcos4
    T1 = *AR0(#( RA_TNI_Obj.rdAgainFlg))
    ; T1 = 1, if one more channel is to be processed
    XAR0 = dbl(*AR0(#(RA_TNI_Obj.dmaChannel_0) )) 
    ; XAR0 (channel) = objPtr->dmaChannel_0    
	 bit(ST2, #15) = #1
    ; set the ARMS bit for client
    ...
    if( T1 != 0 )  call _ACPY2_start
    ; one more channel is expected, please read in 
