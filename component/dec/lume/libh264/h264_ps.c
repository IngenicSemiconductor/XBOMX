...
#include <assert.h>
#ifdef JZC_MAU_OPT
#include "jzsoc/mem_def_falcon.h"
#endif
...
build_qp_table(PPS *pps, int t, int index)
{
    int i;
    for(i = 0; i < 255; i++)
        pps->chroma_qp_table[t][i & 0xff] = ff_h264_chroma_qp[av_clip(i + index, 0, 51)];
}
#endif
...
#ifdef JZC_MAU_OPT
    memcpy(CHR_QT_TAB_BASE, pps->chroma_qp_table[0], 256);
    memcpy(CHR_QT_TAB_BASE+256, pps->chroma_qp_table[1], 256);
#endif
