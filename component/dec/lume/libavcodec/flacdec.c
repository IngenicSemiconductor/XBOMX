                            ...
                            AVPacket *avpkt)
{
    ...
    *data_size=0;
	...
	//	int metadata_return = get_metadata_size(buf, buf_size);
	//	printf("%d\n", metadata_return);
...
#undef fprintf
    static aaframe=0;
    {
      static FILE * flacfp=NULL;
      p = s->decoded[0];
      if(aaframe==0)
	flacfp=fopen("flac_dump","w+");
      fprintf(flacfp,"====================aaframe=%d,output_size=%d====================\n",aaframe,output_size);
      {
	int iii;
	for(iii=0;iii<output_size/4;iii+=8){
	  fprintf(flacfp,"0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n", p[iii+0],p[iii+1],p[iii+2],p[iii+3],p[iii+4],p[iii+5],p[iii+6],p[iii+7]);
	}
...
#define DECORRELATE(left, right)\
    ...
    case FLAC_CHMODE_INDEPENDENT:
	  ...
	  if(channels == 2 && s->channels > 2){
	    if(s->is32){
	      samples_32[2*j] = samples_32[2*j+1] = s->decoded[0][j] << s->sample_shift;
	      for (i = 0; i < s->channels; i++) 
		samples_32[2*j] = samples_32[2*j+1] = (samples_32[2*j+1] + (s->decoded[i][j] << s->sample_shift)) >> 1;
	    }else{
	      samples_16[2*j] = samples_16[2*j+1] = s->decoded[0][j] << s->sample_shift;
	      for (i = 0; i < s->channels; i++)
		samples_16[2*j] = samples_16[2*j+1] = (samples_16[2*j+1] + (s->decoded[i][j] << s->sample_shift)) >> 1;
	    }
	  }else{
            for (i = 0; i < s->channels; i++) {
                if (s->is32)
    ...
    static aaframe=0;
    {
      ...
      if(aaframe==0)
	flacfp=fopen("flac_dump","w+");
      fprintf(flacfp,"====================aaframe=%d====================\n",aaframe);
      {
	int iii;
	for(iii=0;iii<output_size/2;iii+=8){
	  fprintf(flacfp,"0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x,0x%x\n", p[iii+0],p[iii+1],p[iii+2],p[iii+3],p[iii+4],p[iii+5],p[iii+6],p[iii+7]);
	}
      }
    }
