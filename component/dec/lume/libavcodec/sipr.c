                             ...
                             int *data_size, AVPacket *avpkt)
{
    ...
    *data_size = mode_par->frames_per_packet * subframe_size *
    ...
    /* Sample format float to s16 conversion */
    data -= mode_par->frames_per_packet * subframe_size * mode_par->subframe_count;
    for(i = 0; i < (*data_size)/4; i++)
      ((int16_t*)datap)[i] = av_clip_int16(lrintf(32768.0 * data[i]));
