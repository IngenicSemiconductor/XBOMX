...
static ChannelElement *get_che(AACContext *ac, int type, int elem_id)
{
    ...
    */
    ...
    if (ac->tags_mapped >= tags_per_config[ac->m4ac.chan_config]&&type!=TYPE_SCE) {
        return NULL;
    }
...
#define AAC_INIT_VLC_STATIC(num, size, ac)			\
    INIT_VLC_STATIC(&ac->vlc_spectral[num], 8, ff_aac_spectral_sizes[num], \
...
static av_cold int aac_decode_init(AVCodecContext *avctx)
{
  //  aac_real_samplerate = 0;
    ...
    AAC_INIT_VLC_STATIC( 0, 304, ac);
    AAC_INIT_VLC_STATIC( 1, 270, ac);
    AAC_INIT_VLC_STATIC( 2, 550, ac);
    AAC_INIT_VLC_STATIC( 3, 300, ac);
    AAC_INIT_VLC_STATIC( 4, 328, ac);
    AAC_INIT_VLC_STATIC( 5, 294, ac);
    AAC_INIT_VLC_STATIC( 6, 306, ac);
    AAC_INIT_VLC_STATIC( 7, 268, ac);
    AAC_INIT_VLC_STATIC( 8, 510, ac);
    AAC_INIT_VLC_STATIC( 9, 366, ac);
    AAC_INIT_VLC_STATIC(10, 462, ac);
    ...
    INIT_VLC_STATIC(&ac->vlc_scalefactors,7,FF_ARRAY_ELEMS(ff_aac_scalefactor_code),
    ...
    for (g = 0; g < ics->num_window_groups; g++) {
        for (i = 0; i < ics->max_sfb;) {
            ...
            if (band_type[idx] == ZERO_BT) {
                ...
                for (; i < run_end; i++, idx++) {
                    offset[2] += get_vlc2(gb, ac->vlc_scalefactors.table, 7, 3) - 60;
                    if (offset[2] > 255U) {
                        av_log(ac->avctx, AV_LOG_ERROR,
                               "%s (%d) out of range.\n", sf_str[2], offset[2]);
                        return -1;
                ...
                for (; i < run_end; i++, idx++) {
                    if (noise_flag-- > 0)
                        ...
                        offset[1] += get_vlc2(gb, ac->vlc_scalefactors.table, 7, 3) - 60;
                    if (offset[1] > 255U) {
                        av_log(ac->avctx, AV_LOG_ERROR,
                               "%s (%d) out of range.\n", sf_str[1], offset[1]);
                        return -1;
                ...
                for (; i < run_end; i++, idx++) {
                    offset[0] += get_vlc2(gb, ac->vlc_scalefactors.table, 7, 3) - 60;
                    if (offset[0] > 255U) {
                        av_log(ac->avctx, AV_LOG_ERROR,
                               "%s (%d) out of range.\n", sf_str[0], offset[0]);
                        return -1;
    ...
    for (g = 0; g < ics->num_window_groups; g++) {
        ...
        for (i = 0; i < ics->max_sfb; i++, idx++) {
            ...
            if (cbt_m1 >= INTENSITY_BT2 - 1) {
                ...
                VLC_TYPE (*vlc_tab)[2] = ac->vlc_spectral[cbt_m1].table;
                OPEN_READER(re, gb);
...
static int decode_cce(AACContext *ac, GetBitContext *gb, ChannelElement *che)
{
    ...
    for (c = 0; c < num_gain; c++) {
        ...
        if (c) {
            ...
            gain = cge ? get_vlc2(gb, ac->vlc_scalefactors.table, 7, 3) - 60: 0;
            ...
            for (g = 0; g < sce->ics.num_window_groups; g++) {
                for (sfb = 0; sfb < sce->ics.max_sfb; sfb++, idx++) {
                    if (sce->band_type[idx] != ZERO_BT) {
                        if (!cge) {
                            int t = get_vlc2(gb, ac->vlc_scalefactors.table, 7, 3) - 60;
                            if (t) {
                                int s = 1;
                                t = gain += t;
                                if (sign) {
...
static av_always_inline int float_to_int16_one1(int_fast32_t tmp){
  // int_fast32_t tmp = *(const int32_t*)src;
    if(tmp & 0xf0000){
        tmp = (0x43c0ffff - tmp)>>31;
        // is this faster on some gcc/cpu combinations?
...
static void float_to_int16_interleave(int16_t *dst, const float **src, long len, int channels,int chan_cfg){
    int i,j,c;
    //   ALOGE("channels=%d,int true_cfg_mono = %d",channels,chan_cfg);
    if(chan_cfg == 1){
      if (channels == 2){
	for(i=0; i<len; i++){
	  dst[2*i] = dst[2*i+1] =  float_to_int16_one(src[0]+i);
	}
      }else{
	for(i=0; i<len; i++){
	...
	int_fast32_t tmp1 = (*(const int32_t*)(src[2]+i) + (*(const int32_t*)(src[1]+i) +  *(const int32_t*)(src[3]+i)) >> 1) >> 1;
	int_fast32_t tmp2 = (*(const int32_t*)(src[2]+i) + (*(const int32_t*)(src[0]+i) +  *(const int32_t*)(src[4]+i)) >> 1) >> 1;
	
	dst[j] =  float_to_int16_one1(tmp1);
	dst[j+1] =  float_to_int16_one1(tmp2) ;
      }
    }
}
                            ...
                            int *data_size, AVPacket *avpkt)
{
  EL("");
    ...
    //aac_real_samplerate = avctx->sample_rate;
    //EL("aac_real_samplerate=%d,samples=%d,avctx->channels=%d",aac_real_samplerate,samples,avctx->channels);
    ...
    *data_size = data_size_tmp;
    ...
    EL("*data_size=%d",*data_size);
    // ALOGE("avctx->channels = %d",avctx->channels);
     avctx->channels = avctx->channels>=6 ? 2 : avctx->channels;
    if (samples){
      float_to_int16_interleave(data, (const float **)ac->output_data, samples,avctx->channels,true_cfg_mono?1:ac->m4ac.chan_config);
    }
