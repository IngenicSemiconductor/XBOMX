...
const int32_t * const wma_sine_windows[] = {
    NULL, NULL, NULL, NULL, NULL, // unused
    ff_sine_32 , ff_sine_64 ,
    aac_sine_short_128, wma_sine_256, wma_sine_512, aac_sine_long_1024, wma_sine_2048, ff_sine_4096
};
...
int ff_wma_init(AVCodecContext *avctx, int flags2)
{
    ...
    for (i = 0; i < s->nb_block_sizes; i++) {
        ...
        s->windows[i] = wma_sine_windows[s->frame_len_bits - i];
     }
    ...
    if (s->use_noise_coding) {
        ...
        if (s->use_exp_vlc) {
	   s->noise_mult = WMA_CONST(0.02);
        } else {
	   s->noise_mult = WMA_CONST(0.04);
        }
...
#else
        {
            ...
            seed = 1;
            if (s->use_exp_vlc) {
              norm = (1.0 / (float)(1LL << 31)) * sqrt(3) * 0.02;
            } else {            
              norm = (1.0 / (float)(1LL << 31)) * sqrt(3) * 0.04;
	    }
            for (i = 0; i < NOISE_TAB_SIZE; i++) {
                seed = seed * 314159 + 1;
                s->noise_table[i] = WMA_CONST((float)((int)seed) * norm);
            }
        }
#endif
    }
...
{
    int code, level, sign;
    //const uint16_t *ilvl = level_table;
    //int16_t *iptr = ptr;
