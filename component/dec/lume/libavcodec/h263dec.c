...
av_cold int ff_h263_decode_init(AVCodecContext *avctx)
{
    ...
    //avctx->hwaccel= ff_find_hwaccel(avctx->codec->id, avctx->pix_fmt);//gjwang
...
av_log(avctx, AV_LOG_DEBUG, "%"PRId64"\n", rdtsc()-time);
...
#ifdef JZC_PMON_P0
    {
      ...
      //static long long int avg_pmon = 0;
      pmon_p0_frmcnt ++;
	...
	printf("PMON nfl:%s\t(size: %d x %d; mb_num: %d) \n",filename,s->width,s->height,mb_num);
      ...
      printf("PMON PARSE -D: %d; I:%d\n",
	      parse_pmon_val/mb_num, parse_pmon_val_ex/mb_num);
      printf("PMON DMB -D: %d; I:%d\n",
	      dmb_pmon_val/mb_num, dmb_pmon_val_ex/mb_num);
      printf("PMON WAIT -D: %d; I:%d\n",
	      wait_pmon_val/mb_num, wait_pmon_val_ex/mb_num);

      parse_pmon_val=0; parse_pmon_val_ex=0;
      dmb_pmon_val=0; dmb_pmon_val_ex=0;
      wait_pmon_val=0; wait_pmon_val_ex=0;
	...
	crc_code2 = crc(s->current_picture.data[0]+crc_i*s->linesize, 
		       avctx->width, crc_code2);
      for(crc_i=0;crc_i<(avctx->height>>1);crc_i++){
	crc_code2 = crc(s->current_picture.data[1]+crc_i*s->uvlinesize, 
		       (avctx->width>>1), crc_code2);
	crc_code2 = crc(s->current_picture.data[2]+crc_i*s->uvlinesize, 
		       (avctx->width>>1), crc_code2);
      }
