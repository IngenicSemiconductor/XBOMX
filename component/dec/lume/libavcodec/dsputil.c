...
static void avg_pixels8_mxu(uint8_t *dst, const uint8_t *src, int stride,int h){
  uint32_t src_aln,src_rs;
  int i;
  src_aln = (uint32_t)src & 0xfffffffc;
...
static void avg_pixels16_mxu(uint8_t *dst, const uint8_t *src, int stride,int h){

  uint32_t src_aln,src_rs;
  int i;
  src_aln = (uint32_t)src & 0xfffffffc;

  if((uint32_t)src & 3){//(i doubt that is needed but ...)
    src_rs = 4 - ((uint32_t)src & 3);
...
#if 0
  //  if(1 || ((uint32_t)src) & 3){//in case src is NOT 4 byte aligned (i doubt that is needed but ...)
    ...
    const int strength= ff_h263_loop_filter_strength[qscale];
    int diff[2];
    short int *pdiff = (short int *)diff;

    int diff1[2];
    short int *pdiff1 = (short int *)diff1;

    uint32_t  source_aln;
    source_aln = ((uint32_t)src - 3*stride);   

    for(x=0; x<8; x += 4){
        S32LDIV(xr1, source_aln, stride, 0);
        S32LDIV(xr3, source_aln, stride, 0);
        S32LDIV(xr5, source_aln, stride, 0);
        S32LDIV(xr7, source_aln, stride, 0);

	Q8ADDE_SS(xr10, xr1, xr7, xr9);//p0 - p3;
	Q8ADDE_SS(xr12, xr5, xr3, xr11);//p2 -p1;
	Q16SLL(xr12, xr12, xr11, xr11, 2);//4*(p2 - p1)

	Q16ADD_AA_WW(xr0, xr11, xr9, xr11);//(p03 + 4*p21);
	Q16ADD_AA_WW(xr0, xr12, xr10, xr12);

	//FIXME: when minus ,the result is not correct 
	Q16SAR(xr12, xr12, xr11, xr11, 3);//(p03 + 4*p21)/8;
	...
	int i;
	for(i = 0; i < 4; i++){
          if     (pdiff[i]<-2*strength) pdiff1[i]= 0;
          else if(pdiff[i]<-  strength) pdiff1[i]=-2*strength - pdiff[i];
          else if(pdiff[i]<   strength) pdiff1[i]= pdiff[i];
          else if(pdiff[i]< 2*strength) pdiff1[i]= 2*strength - pdiff[i];
          else                   pdiff1[i]= 0;	  	  
	}
	...
	Q16SAT(xr14, xr15, xr14);

	Q8ADDE_AA(xr4, xr3, xr14, xr3);//p1 += d1;
	Q8ADDE_SS(xr6, xr5, xr14, xr5);//p2 -= d1;

	Q16SAT(xr3, xr4, xr3);
	Q16SAT(xr5, xr6, xr5);
	
	{
	D16CPS(xr14, xr14, xr14);         //ad1 = FFABS(d1)
	D16CPS(xr15, xr15, xr15);         //ad1 = FFABS(d1)

	Q16SAR(xr15, xr15, xr14, xr14, 1);  //ad1 = FFABS(d1)>>1;

	Q16SAR(xr10, xr10, xr9, xr9, 2);//FIXME: NOT correct, ((p0-p3)/4)

	S32I2M(xr12, -1);

	D16CPS(xr13, xr14, xr12);//xr13 = -ad1
	D16MAX(xr11, xr13, xr9);
	D16MIN(xr11, xr14, xr11);//xr11 = d2= av_clip((p0-p3)/4, -ad1, ad1);

	D16CPS(xr13, xr15, xr12);//xr13 = -ad1
	D16MAX(xr6, xr13, xr10);
	D16MIN(xr6, xr15, xr6);
	
      	Q16SAT(xr11, xr6, xr11);//xr11 = d2

	Q8ADDE_SS(xr2, xr1, xr11, xr1);//p0 = p0 - d2;
	Q8ADDE_AA(xr8, xr7, xr11, xr7);//p3 = p3 + d2;

	Q16SAT(xr1, xr2, xr1);
	Q16SAT(xr7, xr8, xr7);
	}

	S32STD(xr7, source_aln, 0);//src[x+  stride] = p3 + d2;
	S32SDIV(xr5, source_aln, -stride, 0);//src[x+0*stride] = p2;
	S32SDIV(xr3, source_aln, -stride, 0);//src[x-1*stride] = p1;
	S32SDIV(xr1, source_aln, -stride, 0);//src[x-2*stride] = p0 - d2;
...
short coef_tab[12]={W1,W7,W5,W3,W2,W6,W0,4,1<<7,0,1<<13,0};
#ifdef JZC_MXU_OPT
static void wmv2_idct_row(short * b)
{
    ...
    S32LDD(xr9,b,0x8);
    S32LDD(xr15,b,0x4);
    D16MUL_HW(xr4,xr9,xr12,xr6);
    S32LDD(xr10,b,0xc);
    D16MAC_AS_HW(xr6,xr15,xr12,xr4);
    D16MUL_LW(xr7,xr15,xr13,xr3);
    D16MAC_AS_LW(xr3,xr10,xr13,xr7);
    S32LDD(xr15,b,0x0);
    D16MUL_HW(xr8,xr15,xr11,xr2);
    D16MAC_AS_HW(xr2,xr10,xr11,xr8);
    S16LDD(xr15,b,0x8,1);
    D16MUL_LW(xr5,xr14,xr15,xr1);
    ...
    a7=S32M2I(xr8);
    /*step 2*/
    a7 = (181*a7+128)>>8;//1,3,5,7,
    a5 = (181*a5+128)>>8;
    S32I2M(xr8,a7);
    S32I2M(xr6,a5);
    D32ADD_AS(xr5,xr5,xr8,xr8);
    S32LDD(xr15,coef_tab,0x10);
    D32ADD_AS(xr6,xr7,xr6,xr7);
    /*step 3*/
    D32ASUM_AA(xr1,xr15,xr15,xr2);
    D32ASUM_AA(xr3,xr15,xr15,xr4);
    D32ASUM_AA(xr5,xr15,xr15,xr6);
    D32ASUM_AA(xr7,xr15,xr15,xr8);
    D32SARL(xr1,xr5,xr1,8);
    D32SARL(xr3,xr3,xr6,8);
    D32SARL(xr4,xr7,xr4,8);
    D32SARL(xr2,xr2,xr8,8);
    S32STD(xr1,b,0x0);
    S32STD(xr3,b,0x4);
    S32STD(xr4,b,0x8);
    S32STD(xr2,b,0xc);
...
static void wmv2_idct_col(short * b)
{
    ...
    /*step 1*/
    S16LDD(xr9,b,0x50,1);
    S16LDD(xr15,b,0x30,1);
    D16MUL_HW(xr4,xr9,xr12,xr6);
    S16LDD(xr15,b,0x20,0);
    S16LDD(xr9,b,0x40,0);
    D16MAC_AS_HW(xr6,xr15,xr12,xr4);
    S16LDD(xr10,b,0x60,0);
    S16LDD(xr10,b,0x70,1);
    D16MUL_LW(xr7,xr15,xr13,xr3);
    D16MAC_AS_LW(xr3,xr10,xr13,xr7);
    S16LDD(xr15,b,0x10,1);
    D16MUL_HW(xr8,xr15,xr11,xr2);
    D16MAC_AS_HW(xr2,xr10,xr11,xr8);
    S16LDD(xr9,b,0x0,1);
    S32SFL(xr15,xr0,xr14,xr0,3);
    D16MUL_LW(xr1,xr14,xr9,xr5);
    D32ADD_AS(xr1,xr1,xr5,xr5);

    D32ASUM_AA(xr3,xr15,xr15,xr4);
    D32ASUM_AA(xr2,xr15,xr15,xr6);
    D32ASUM_AA(xr7,xr15,xr15,xr8);
    D32SAR(xr1,xr1,xr2,xr2,3);
    D32SAR(xr3,xr3,xr4,xr4,3);
    D32SAR(xr5,xr5,xr6,xr6,3);
    D32SAR(xr7,xr7,xr8,xr8,3);
    ...
    D32ASUM_AA(xr5,xr15,xr15,xr6);
    D32ASUM_AA(xr7,xr15,xr15,xr8);
    D32SARL(xr1,xr5,xr1,14);
    D32SARL(xr3,xr3,xr6,14);
    D32SARL(xr4,xr7,xr4,14);
    D32SARL(xr2,xr2,xr8,14);
    S16STD(xr1,b,0x0,0);
    S16STD(xr1,b,0x10,1);
    S16STD(xr3,b,0x20,0);
    S16STD(xr3,b,0x30,1);
...
void ff_wmv2_idct_c(short * block){
    ...
    S32LDD(xr11,coef_tab,0x0);
    S32LDD(xr12,coef_tab,0x4);
    S32LDD(xr13,coef_tab,0x8);
    S32LDD(xr14,coef_tab,0xc);
    for(i=0;i<64;i+=8){
