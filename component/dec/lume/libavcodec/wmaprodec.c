...
#define MUL_R(A,B) (int32_t)(((int64_t)(A)*(int64_t)(B)) >> WMA3_BITS)
...
typedef struct WMAProDecodeCtx {
    ...
    DECLARE_ALIGNED(16, int32_t, tmp)[WMAPRO_BLOCK_MAX_SIZE]; ///< IMDCT output buffer
...
const int32_t * const wma3_sine_windows[] = {
    NULL, NULL, NULL, NULL, NULL, // unused
    ff_sine_32 , ff_sine_64 ,
    aac_sine_short_128, wma_sine_256, wma_sine_512, aac_sine_long_1024, wma_sine_2048, wma_sine_4096
};
...
static av_cold int decode_init(AVCodecContext *avctx)
{
    ...
    for (i = 0; i < WMAPRO_BLOCK_SIZES; i++)
        wma3_mdct_init(&s->mdct_ctx[i], BLOCK_MIN_BITS+1+i, 1,
    ...
    for (i = 0; i < WMAPRO_BLOCK_SIZES; i++) {
        ...
        s->windows[WMAPRO_BLOCK_SIZES - i - 1] = wma3_sine_windows[win_idx];
    }
...
int ff_wma3_run_level_decode(AVCodecContext* avctx, GetBitContext* gb,
                            VLC *vlc,
                            ...
                            int coef_nb_bits)
{
    ...
    for (; offset < num_coefs; offset++) {
        ...
        if (code > 1) {
            ...
            sign = get_bits1(gb) - 1;
            if(sign<<31) 
              ptr[offset & coef_mask] = -level_table[code];
            else
              ptr[offset & coef_mask] = level_table[code]; 
        } else if (code == 1) {
            /** EOB */
...
unsigned int ff_wma_get_large_val_skip(GetBitContext* gb)
{
    ...
    skip_bits_long(gb, n_bits);
    return 0;
    
}

int ff_wma3_run_level_decode_skip(AVCodecContext* avctx, GetBitContext* gb,
                            VLC *vlc,
                            const uint16_t *level_table, const uint16_t *run_table,
                            int version, int offset,
                            int num_coefs, int block_len, int frame_len_bits,
                            int coef_nb_bits)
{
    int code, level, sign;
    ...
    for (; offset < num_coefs; offset++) {
        ...
        if (code > 1) {
            ...
            /** escape */
            if (!version) {
                skip_bits(gb, coef_nb_bits);
                offset += get_bits(gb, frame_len_bits);
            } else {
                ff_wma_get_large_val_skip(gb);
                /** escape decode */
                if (get_bits1(gb)) {
                    if (get_bits1(gb)) {
                        if (get_bits1(gb)) {
...
static int decode_coeffs(WMAProDecodeCtx *s, int c)
{
    ...
    if (rl_mode) {
        ...
        if (ff_wma3_run_level_decode(s->avctx, &s->gb, vlc,
...
static int decode_coeffs_skip(WMAProDecodeCtx *s, int c)
{
    static const int fval_tab[16] = {
        0,  1,  2, 3, 
        4,  5,  6, 7, 
    ...
    /** decode run level coded coefficients */
    if (rl_mode) {
        if (ff_wma3_run_level_decode_skip(s->avctx, &s->gb, vlc,
                                    level, run, 1,
                                    cur_coeff, s->subframe_len,
                                    s->subframe_len, s->esc_len, 0))
            return AVERROR_INVALIDDATA;
...
static void vector_fmul_scalar_c(int32_t *dst, const int32_t *src, int32_t mul,
                                 int len)
{
    int i;
    for (i = 0; i < len; i++)
       dst[i] = MUL_R(src[i] , mul);
}
            ...
            for (sfb = s->cur_sfb_offsets;
		        ...
		        const int32_t* mat = s->chgroup[i].decorrelation_matrix;
                        ...
                        for (ch = ch_data; ch < ch_end; ch++) {
                            data_ptr = data;
			    S32I2M(xr1,0);
			    for(xx=0;xx<num_channels;xx++)
			      {
                                S32MUL(xr2,xr3,*data_ptr++,*mat++); 
                                S32EXTRV(xr2,xr3,18,31);
                                D32SLL(xr4,xr2,xr0,xr0,1);
				D32ASUM_AA(xr1,xr4,xr0,xr0);
			      }
                            (*ch)[y] = S32M2I(xr1);
                        }
                    }
                    ...
                    vector_fmul_scalar_c(ch_data[0] + sfb[0],
                                              ch_data[0] + sfb[0],
					      23168, len);
                    vector_fmul_scalar_c(ch_data[1] + sfb[0],
                                              ch_data[1] + sfb[0],
					      23168, len);
                }
            }
...
void wma3_vector_fmul_window_c(int32_t *dst, const int32_t *src0, const int32_t *src1, const int32_t *win, int len){
#if 0
    int i,j;
    dst += len;
    win += len;
    ...
    for(i=-len, j=len-1; i<0; i++, j--) {
        ...
        int32_t wi = win[i];
        int32_t wj = win[j];
        dst[i] = MUL_F(s0,wj) - MUL_F(s1,wi);
        dst[j] = MUL_F(s0,wi) + MUL_F(s1,wj);
    }
#else
    int i,j;
    dst += len;
    ...
    src0+= len;
    for(i=-len, j=len-1; i<0; i++, j--) {
      S32MUL(xr1,xr2, src0[i], win[j]);
      S32MUL(xr3,xr4, src0[i], win[i]);
      S32MSUB(xr1,xr2,src1[j],win[i]);
      S32MADD(xr3,xr4,src1[j],win[j]);
 ...
 *@param s codec context
 */
static void wmapro_window(WMAProDecodeCtx *s,int chan)
{
    int i;
    if (chan==6){
      for (i = 0; i < s->channels_for_cur_subframe; i++) {
        ...
        if (c==1||c==2){
          ...
          int32_t* start = s->channel[c].coeffs - (winlen >> 1);
          ...
          wma3_vector_fmul_window_c(start, start, start + winlen,
                                  window, winlen);
...
static void wma3_vector_fmul_scalar_c(int32_t *dst, const int32_t *src, int32_t mul,
                                 int len)
{
    int i;
#if 0
    ...
    for (i = 0; i < len; i+=4)
      {
        S32MUL(xr1,xr2,src[i],mul);
        S32MUL(xr3,xr4,src[i+1],mul); 
        S32MUL(xr5,xr6,src[i+2],mul); 
        S32MUL(xr7,xr8,src[i+3],mul); 
        S32SDI(xr2,dst,4);
        S32SDI(xr4,dst,4); 
        S32SDI(xr6,dst,4); 
        S32SDI(xr8,dst,4);   
...
static int decode_subframe(WMAProDecodeCtx *s)
{
    ...
    */
    ...
    if (transmit_coeffs) {
      ...
      if (s->num_channels==6){
          for (i = 0; i < s->channels_for_cur_subframe; i++) {
	        ...
	        //const int32_t quant = pow(10.0, exp / 20.0);
	        const int32_t quant = pow_table[exp];
                int start = s->cur_sfb_offsets[b];
                wma3_vector_fmul_scalar_c(s->tmp + start,
                                          s->channel[c].coeffs + start,
                                          quant, end - start);
              }
              /** apply imdct (ff_imdct_half == DCTIV with reverse) */
              wma3_imdct_half_c(&s->mdct_ctx[av_log2(subframe_len) - BLOCK_MIN_BITS],
    ...
    wmapro_window(s,s->num_channels);
...
static int decode_frame(WMAProDecodeCtx *s)
{
    ...
    if(s->num_channels == 6){
       int16_t* ptr  = (int16_t*)s->samples;
        //int incr = s->num_channels;
        int32_t* iptr0 = s->channel[0].out;
        int32_t* iptr1 = s->channel[1].out;
        int32_t* iptr2 = s->channel[2].out;
        int32_t* iptr3 = s->channel[3].out;
        int32_t* iptr4 = s->channel[4].out;
        int32_t* iptr5 = s->channel[5].out;

        int32_t* iend = iptr0 + s->samples_per_frame;
        // FIXME should create/use a DSP function here
        while (iptr0 < iend) {
	  ptr[0] = ptr[1] =  (int16_t)(((*iptr0++)+(*iptr1++)+(*iptr2++)+(*iptr3++)+(*iptr4++)+(*iptr5++))/(6*4096));
	  ptr += 2;
        }
	...
	for (i = 0; i < s->num_channels; i++) {	  
	  memcpy(&s->channel[i].out[0],
       ...
       for (i = 0; i < s->num_channels; i++) {
        int16_t* ptr  = (int16_t*)s->samples + i;
        int incr = s->num_channels;
    ...
    if (s->skip_frame) {
        ...
        s->samples += (s->num_channels > 2 ? 2 : s->num_channels) * s->samples_per_frame / 2;
